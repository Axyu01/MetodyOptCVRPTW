...
void Tests::EvoTest(Problem *problem, std::string test_id, int popSize, int Xp,
                    int Mp, int turSize, int elitesNum, int MUT_ID,
                    int CROSS_ID) {
  // Init Loggers
...
void Tests::EvoParamTest() {
  ...
  for (int i = 0; i < 10; i++)
    EvoTest(problem, test_id + "_" + to_string(MUT_ID) + "_" + to_string(i),
            popSize, Xp, Mp, turSize, elitesNum, MUT_ID, CROSS_ID);
  delete problem;
  ...
  CROSS_ID = CrossOps::OX_ID; // CrossOps::OX_ID,CrossOps::CX_ID
  MUT_ID =
      MutationOps::INVERSE_ID; // MutationOps::SWAP_ID,MutationOps::INVERSE_ID
...
void Tests::EvoTest() {
  ...
  for (int i = 0; i < 10; i++)
    EvoTest(problem, to_string(i), popSize, Xp, Mp, turSize, elitesNum, MUT_ID,
            CROSS_ID);
  delete problem;
...
void Tests::SATest(Problem *problem, std::string test_id, float startTemp,
                   float coolingFactor, int tempIterations, int MUT_ID) {
  // Init Logger
  ...
  while (sa.Iterate()) {
    if (best != sa.bestSolution->eval) {
      ...
      // cout <<endl<<sa.currentIteration<<"/"<<sa.maxIterations<<" | temp:
      // "<<sa.currentTemp<<" |new best:"<<best<<endl;
    }
...
void Tests::SAParamTest() {
  ...
  Problem *problem = new Problem("problems/tai20_10_0.fsp", 20);
  budget = problem->PREFFERED_GENOME_SIZE * problem->PREFFERED_GENOME_SIZE *
  ...
  for (int i = 0; i < 10; i++)
    SATest(problem, test_id + "_" + to_string(MUT_ID) + "_" + to_string(i),
           startTemp, coolingFactor, tempIterations, MUT_ID);
  MUT_ID = MutationOps::INVERSE_ID;
  for (int i = 0; i < 10; i++)
    SATest(problem, test_id + "_" + to_string(MUT_ID) + "_" + to_string(i),
           startTemp, coolingFactor, tempIterations, MUT_ID);
...
void Tests::SATest(Problem *problem) {
  ...
  while (sa.Iterate()) {
    if (best != sa.bestSolution->eval) {
      best = sa.bestSolution->eval;
      cout << endl
           << sa.currentIteration << "/" << sa.maxIterations
           << " | temp: " << sa.currentTemp << " |new best:" << best << endl;
    }
    logger.Log(sa.currentSolution);
  }
  sa.bestSolution->print();
...
void Tests::SATest() {
  ...
  int MUT_ID = MutationOps::SWAP_ID;
  Problem *problem = new Problem("problems/tai4_5_0.fsp", 4);
  startTemp = problem->PREFFERED_GENOME_SIZE * problem->PREFFERED_GENOME_SIZE *
              problem->PREFFERED_GENOME_SIZE;
  for (int i = 0; i < 10; i++)
    SATest(problem, to_string(i), startTemp, coolingFactor, tempIterations,
           MUT_ID);
  delete problem;
