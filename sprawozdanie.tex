...
\begin{description}
    ...
    \item[Mutacja] Stosowana z prawdopodobieństwem \texttt{Mp}:
          ...
          \end{itemize}
    \item[Elityzm] \texttt{elitesNum} najlepszych osobników przechodzi do nowej
...
Kolejność ma znaczenie: redistribucja zmienia strukturę tras, naprawa eliminuje
...
funkcji celu. Każda konfiguracja testowana $N=5$ razy niezależnie.
Miara jakości: średnia wartość funkcji celu z ostatniej generacji.
...
-ffast-math}. Uruchomienia równoległe z użyciem \texttt{std::async} --
w porównaniu końcowym 60 wątków jednocześnie (12 instancji $\times$ 5 uruchomień).
Generator losowy: \texttt{thread\_local std::mt19937} (niezależny dla każdego wątku).
...
\begin{table}[H]
\centering
\caption{Wyniki porównania końcowego -- średni koszt z 5 uruchomień}
\begin{tabular}{lrrl}
\toprule
Instancja & ops\_on & ops\_off & Zwycięzca \\
\midrule
C101  & 1414 & \textbf{1360} & ops\_off ($-3{,}8\%$) \\
C102  & 1417 & \textbf{1303} & ops\_off ($-8{,}0\%$) \\
C201  & \textbf{962}  & 1148 & ops\_on ($-16{,}2\%$) \\
C202  & \textbf{1189} & 1191 & ops\_on ($-0{,}2\%$) \\
R101  & \textbf{1256} & 1265 & ops\_on ($-0{,}7\%$) \\
R102  & 1234 & \textbf{1204} & ops\_off ($-2{,}4\%$) \\
R201  & \textbf{1188} & 1200 & ops\_on ($-1{,}0\%$) \\
R202  & 1181 & \textbf{1143} & ops\_off ($-3{,}2\%$) \\
RC101 & 1543 & \textbf{1487} & ops\_off ($-3{,}6\%$) \\
RC102 & \textbf{1522} & 1561 & ops\_on ($-2{,}5\%$) \\
RC201 & 1418 & \textbf{1367} & ops\_off ($-3{,}6\%$) \\
RC202 & 1304 & \textbf{1298} & ops\_off ($-0{,}5\%$) \\
\midrule
\textbf{OVERALL} & 1302 & \textbf{1294} & \textbf{ops\_off ($-0{,}7\%$)} \\
\bottomrule
\end{tabular}
\end{table}
...
Ogólnie EA bazowy (ops\_off) osiągnął nieznacznie lepszy wynik niż EA z operatorami
(ops\_on): średni koszt 1294 vs 1302 (różnica 0{,}7\%). Wynik ten mieści się jednak
w granicach zmienności statystycznej przy $N=5$ -- w poprzednim uruchomieniu
(identyczna konfiguracja) ops\_on wygrał z podobną przewagą. Oba warianty są
praktycznie równoważne globalnie, jednak wyraźnie różnią się zależnie od rodziny instancji:

\begin{itemize}
    \item \textbf{C1 (clustered, wąskie TW):} ops\_off wygrywa na obu instancjach
          o 4--8\%. Klastry klientów z wąskimi oknami tworzą naturalne trasy,
          które EA bazowy odkrywa przez krzyżowanie OX (zachowujące kolejność względną).
          Operatory niestandardowe, zwłaszcza RedistributeLocations, mogą rozbijać
          te naturalne klastry, a Repair zamiast je naprawiać -- niszczy ich strukturę.
    \item \textbf{C2 (clustered, szerokie TW):} ops\_on wygrywa wyraźnie -- C201 o ponad
          16\%. Szerokie okna dają operatorom swobodę działania -- Repair rzadziej
          zaburza trasę, a OptimizeTracks może efektywnie dopracować kolejność w klastrze.
    \item \textbf{R1/R2 (random):} różnice są minimalne (poniżej 3\%), wyniki
          przeplatają się -- żadna metoda nie dominuje. Losowe rozmieszczenie klientów
          nie preferuje żadnego ze stylów optymalizacji.
    \item \textbf{RC1/RC2 (mixed):} wyniki mieszane -- ops\_off wygrywa RC101 i RC201,
          ops\_on wygrywa RC102. Mieszane instancje nie faworyzują jednoznacznie
          żadnego podejścia.
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Rodzina instancji determinuje przydatność operatorów.}
          Nie ma jednej metody dominującej we wszystkich typach. Różnica globalna
          (0{,}7\%) jest w granicach szumu statystycznego -- w zależności od losowości
          uruchomienia wygrać może każdy z wariantów. Natomiast na poziomie rodziny
          instancji wzorce są powtarzalne: ops\_off wygrywa C1, ops\_on wygrywa C2.
          Typ instancji powinien informować wybór metody.
...
Oficjalne wartości BKS dla benchmarku Solomona (źródło: SINTEF Top Results)
wymagają \textbf{pełnej dopuszczalności} -- wszystkie okna czasowe muszą być
...
\begin{table}[H]
\centering
\caption{Porównanie z BKS -- wyniki ops\_off (lepszy wariant globalnie)}
\begin{tabular}{lrrrrr}
\toprule
Instancja & BKS & ops\_off avg & gap (\%) & ops\_on avg & gap (\%) \\
\midrule
C101  & 828{,}94  & 1360 & $+64{,}1\%$ & 1414 & $+70{,}6\%$ \\
C102  & 828{,}94  & 1303 & $+57{,}2\%$ & 1417 & $+70{,}9\%$ \\
C201  & 591{,}56  & 1148 & $+94{,}1\%$ & 962  & $+62{,}7\%$ \\
C202  & 591{,}56  & 1191 & $+101{,}3\%$& 1189 & $+101{,}0\%$\\
R101  & 1650{,}80 & 1265 & $-23{,}4\%$ & 1256 & $-23{,}9\%$ \\
R102  & 1486{,}12 & 1204 & $-19{,}0\%$ & 1234 & $-16{,}9\%$ \\
R201  & 1252{,}37 & 1200 & $-4{,}2\%$  & 1188 & $-5{,}1\%$  \\
R202  & 1191{,}70 & 1143 & $-4{,}1\%$  & 1181 & $-0{,}9\%$  \\
RC101 & 1696{,}95 & 1487 & $-12{,}4\%$ & 1543 & $-9{,}1\%$  \\
RC102 & 1554{,}75 & 1561 & $+0{,}4\%$  & 1522 & $-2{,}1\%$  \\
RC201 & 1406{,}94 & 1367 & $-2{,}8\%$  & 1418 & $+0{,}8\%$  \\
RC202 & 1365{,}65 & 1298 & $-4{,}9\%$  & 1304 & $-4{,}5\%$  \\
\bottomrule
\end{tabular}
\end{table}
